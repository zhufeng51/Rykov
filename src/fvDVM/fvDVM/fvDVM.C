...
void Foam::fvDVM::updateMacroSurf()
{
    ...
    Ttsurf_ = dimensionedScalar("0", Ttsurf_.dimensions(), 0);
    Trsurf_ = dimensionedScalar("0", Trsurf_.dimensions(), 0);
    qtSurf_ = dimensionedVector("0", qtSurf_.dimensions(), vector(0, 0, 0));
    qrSurf_ = dimensionedVector("0", qrSurf_.dimensions(), vector(0, 0, 0));
    ...
    surfaceScalarField rhoErsurf = rhoSurf_*R_*Trsurf_;
   ...
   forAll(DV_, dvi)
    {
        ...
        rhoEsurf  += dXiCellSize_*dv.weight()
           *(
                0.5*(dv.gSurf()*magSqr(dv.xi()) 
              + dv.hSurf()) + dv.rSurf()
            );
        rhoErsurf += dXiCellSize_*dv.weight()*dv.rSurf();
    }
    ...
    if (mpiReducer_.dvParallel()&& mpiReducer_.npd() < mpiReducer_.nproc()) 
    {
        ...
        mpiReducer_.reduceField(rhoErsurf);
    }

    //- get Prim. from Consv.
    Usurf_ = rhoUsurf/rhoSurf_;

    Tsurf_ = (2.*rhoEsurf - rhoSurf_*magSqr(Usurf_))/((KInner_ + 3)*R_*rhoSurf_);

    Trsurf_ = (Z_*tauSurf_*rhoErsurf + 0.5*time_.deltaT()*rhoSurf_*R_*Tsurf_)/(Z_*tauSurf_ + 0.5*time_.deltaT())/rhoSurf_/R_;
    
    Ttsurf_ = (5.0*Tsurf_ - 2.0*Trsurf_)/3.0;

    updateTau(tauSurf_, Ttsurf_, rhoSurf_);
    //- peculiar vel.
    ...
    forAll(DV_, dvi)
    {
        discreteVelocity& dv = DV_[dvi];
        c = dv.xi() - Usurf_;
        qtSurf_ += 0.5*dXiCellSize_*dv.weight()*c
            *(
                magSqr(c)*dv.gSurf() + dv.hSurf()
             );
        qrSurf_ += dXiCellSize_*dv.weight()*c * dv.rSurf();
    }

    //- Get global heat flux, via MPI_Allreuce
    if (mpiReducer_.dvParallel()&& mpiReducer_.npd() < mpiReducer_.nproc())
    {
        mpiReducer_.reduceField(qtSurf_);
        mpiReducer_.reduceField(qrSurf_);
    }
    //- correction for bar to original
   // qtSurf_ = tauSurf_/(tauSurf_ + 0.5*time_.deltaT() + 0.5*time_.deltaT()*(-1./3.)*(1. - 1./Z_ + 1./Z_*omega0_))*qtSurf_;
   // qrSurf_ = tauSurf_/(tauSurf_ + 0.5*time_.deltaT() + 0.5*time_.deltaT()*(delta_ - 1.)*(1. - 1./Z_ + 1./Z_*omega1_))*qrSurf_;
                ...
                forAll(DV_, dvi)
                {
                    ...
                    qPatch[facei] += 0.5*dXiCellSize*dv.weight()*c  //sometimes wall moves, then c != \xi
                        *(
                           ...
                           + dv.rSurf().boundaryField()[patchi][facei]
                         );
                    stressPatch[facei] += 
...
void Foam::fvDVM::updateMacroVol()
{
    rhoold  = rhoVol_;
    Uold = Uvol_;
    Told = (3.*Ttvol_ + 2.*Trvol_)/5.0;
    Trold = Trvol_;
    Ttold = Ttvol_;
    tauold = tauVol_;

    volVectorField rhoUvol = rhoold*Uold;
    volScalarField rhoEvol = rhoold*(magSqr(Uold) + (KInner_ + 3)* R_* Told);
    volScalarField rhoUUvol = 0.5*rhoold*(magSqr(Uold) + (KInner_ + 3)* R_* Told);
    volScalarField rhoErvol = rhoold * R_* Trold;
    ...
    rhoErvol = dimensionedScalar("0", rhoErvol.dimensions(), 0);

    /// update macro flux
    forAll(DV_, dvi)
    {
        discreteVelocity& dv = DV_[dvi];
        rhoVol_ += dXiCellSize_*dv.weight()*dv.gFlux();
        rhoUvol += dXiCellSize_*dv.weight()*dv.gFlux()*dv.xi();
        rhoEvol += dXiCellSize_*dv.weight()
            *(
                0.5*(magSqr(dv.xi())*dv.gFlux() 
                + dv.hFlux()) + dv.rFlux()
             );
        rhoErvol += dXiCellSize_*dv.weight()*dv.rFlux();
    }
    ...
    if (mpiReducer_.dvParallel() && mpiReducer_.npd() < mpiReducer_.nproc())
    {
        ...
        mpiReducer_.reduceField(rhoErvol);
    }
        
    rhoVol_ = rhoold - rhoVol_;                 /// update rho

    Uvol_ = (rhoold*Uold - rhoUvol) / rhoVol_;  /// update rho*U

    Tvol_ = (2.*(rhoUUvol - rhoEvol) - rhoVol_*magSqr(Uvol_))/((KInner_ + 3)*R_*rhoVol_);
    
    // Trvol_ = (Z_*tauold/(Z_*tauold + 0.5*time_.deltaT())
    //         * ((-rhoErvol + (Z_*tauold - 0.5*time_.deltaT())/(Z_*tauold)*rhoold*R_*Trold) 
    //         + 0.5*time_.deltaT()/(Z_*tauold)*(rhoold*R_*Told + rhoVol_*R_*Tvol_)))/rhoVol_/R_;
    Trvol_ = (1./(1 + 0.5*time_.deltaT()/(Z_ * tauVol_)))*
             (rhoold*R_*Trold - rhoErvol + 0.5*time_.deltaT()*(rhoVol_*R_*Tvol_/(Z_*tauVol_) + 
             (rhoold*R_*Told - rhoold*R_*Trold) / (Z_*tauold))) / rhoVol_ / R_;

    Ttvol_ = (5.0*Tvol_ - 2.0*Trvol_)/3.0;
    //- Correct the macro field boundary conition
    Uvol_.correctBoundaryConditions();
    Tvol_.correctBoundaryConditions();
    Trvol_.correctBoundaryConditions();
    Ttvol_.correctBoundaryConditions();
    //- Note for maxwell wall, the operation here update 
    //- the boundary rho field but it's meaningless.
    updateTau(tauVol_, Ttvol_, rhoVol_);

    tauVol_.correctBoundaryConditions();
    /// update the GH distribution
    forAll(DV_, DVid)
    {
        DV_[DVid].updateGHVol();
    }
    ...
    qtVol_ = dimensionedVector("0", qtVol_.dimensions(), vector(0, 0, 0));
    qrVol_ = dimensionedVector("0", qrVol_.dimensions(), vector(0, 0, 0));

    forAll(DV_, dvi)
    {
        discreteVelocity& dv = DV_[dvi];
        c = dv.xi() - Uvol_;
        qtVol_ += 0.5*dXiCellSize_*dv.weight()*c
            *(
                magSqr(c)*dv.gVol() + dv.hVol()
             );
        qrVol_ += dXiCellSize_*dv.weight()*c*dv.rVol();
    }

    //- get global heat flux via MPI_Allreduce
    if (mpiReducer_.dvParallel()&& mpiReducer_.npd() < mpiReducer_.nproc())
    {
        mpiReducer_.reduceField(qtVol_);
        mpiReducer_.reduceField(qrVol_);
    }

    //qtVol_ = (Z_/(2./3.*Z_ + (1-omega0_)*(1./3.))) * qtVol_;
    //qrVol_ = (Z_/(delta_*Z_ + (1-omega1_)*(1 - delta_))) * qrVol_;
    qVol_ = qtVol_ + qrVol_;
}
...
{
    tau = muRef_*exp(omega_*log(Tt/Tref_))/rho/Tt/R_;
}
...
(
 ...
 Foam::argList& args
    ...
    Z_(readScalar(gasProperties_.lookup("Z"))),
    omega0_(readScalar(gasProperties_.lookup("omega0"))),
    omega1_(readScalar(gasProperties_.lookup("omega1"))),
    delta_(readScalar(gasProperties_.lookup("delta"))),
    ...
    )
{
    ...
    updateTau(tauVol_, Ttvol_, rhoVol_); //calculate the tau at cell when init
    Usurf_ = fvc::interpolate(Uvol_, "linear"); // for first time Dt calculation.
    // Tsurf_ = fvc::interpolate(Tvol_, "linear");
    // rhoSurf_ = fvc::interpolate(rhoVol_, "linear");
    // updateTau(tauSurf_, Tsurf_, rhoSurf_);
    dimensionedScalar tauUnit("tauUnit", dimTime, 1);
    tauSurf_ = tauUnit;
    // Trvol_ = Tvol_;
    // Ttvol_ = Tvol_;
    // InitialiseSurfTau();
    Info << " we are in rdugksFoam for Rykov model" << endl;
}
