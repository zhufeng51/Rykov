...
:
    ...
    (
        ...
        dimensionedVector
        (
            "0", rBarPvol_.dimensions()/dimLength, vector(0,0,0)
        ),
        "zeroGradient"
    )   
{
...
void Foam::discreteVelocity::setBCtype()
{
    ...
    forAll(rhoBCs, patchi)
    {
        ...
        if( bcMap.find(rhoBCtype) != bcMap.end() ) // found
        {
            ...
            rSurf_.BOUNDARY_FIELD_REF.set
            (
                patchi, 
                fvsPatchField<scalar>::New
                (
                    bcMap[rhoBCtype], mesh_.boundary()[patchi], rSurf_
                )
            );
        }
    }
...
void Foam::discreteVelocity::initBoundaryField()
{
    ...
    GeometricField<scalar, fvsPatchField, surfaceMesh>::Boundary& 
        rBCs = rSurf_.BOUNDARY_FIELD_REF;
#endif
...
void Foam::discreteVelocity::updateGHvol()
{
    ...
    rBarPvol_.correctBoundaryConditions();
}
...
void Foam::discreteVelocity::updateGHInterpolation()
{
  ...
  // Pre-setup
    ...
    rBarPgrad_.correctBoundaryConditions();
    ...
    forAll(gBarPgrad_.BOUNDARY_FIELD_REF, patchi)
    {
           ...
           ) // only for fixed gradient g/hBarPvol
        {
            ...
            fixedGradientFvPatchField<scalar>& rBarPvolPatch = 
                refCast<fixedGradientFvPatchField<scalar> >
                (rBarPvol_.BOUNDARY_FIELD_REF[patchi]);
            ...
            forAll(gBarPvolPatch, pFacei)
            {
                ...
                rBarPvolPatch.gradient()[pFacei] =
                    rBarPgrad_.BOUNDARY_FIELD_REF[patchi][pFacei]&n[pFacei];
            }
        }
    }
    ...
    forAll(owner, facei)
    {
        ...
        if ((xii&Sf[facei]) >=  VSMALL) // comming from own
        {
            ...
            iRsurf[facei] = iRbarPvol[own]
              + (iRbarPgrad[own]&(Cf[facei] - C[own] - 0.5*xii*dt));
        }
        ...
        else if ((xii&Sf[facei]) < -VSMALL) // comming form nei
        {
            ...
            iRsurf[facei] = iRbarPvol[nei]
              + (iRbarPgrad[nei]&(Cf[facei] - C[nei] - 0.5*xii*dt));  
        }
        else 
        {
            iGsurf[facei] = 0.5*
            ...
            iRsurf[facei] = 0.5*
            (
               iRbarPvol[nei] + ((iRbarPgrad[nei])
               &(Cf[facei] - C[nei] - 0.5*xii*dt))
             + iRbarPvol[own] + ((iRbarPgrad[own])
               &(Cf[facei] - C[own] - 0.5*xii*dt))
            );
        }
    }
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        fvsPatchField<scalar>& rSurfPatch = rSurf_.BOUNDARY_FIELD_REF[patchi];
        ...
        if (type == "zeroGradient")
        {
            ...
            rSurfPatch == rBarPvol_.BOUNDARY_FIELD_REF[patchi].patchInternalField();
        }
        else if (type == "mixed")
        {
            ...
            forAll(gSurfPatch, facei)
            {
                ...
                if ((xii&SfPatch[facei]) > 0 ) // outgoing
                {
                    ...
                    // rSurfPatch[facei] = iRbarPvol[faceCells[facei]];
                    gSurfPatch[facei] = iGbarPvol[faceCells[facei]] 
        ...
        else if (type == "farField")
        {
            ...
            forAll(gSurfPatch, facei)
            {
                ...
                else // incomming, set to be equlibrium, give rho and T, extrapolate U
                {
                    // set to maxwellian
                ...
                else if ((xii&faceSf) <  -VSMALL )//incomming from processor boundaryField
                {
                    ...
                    hSurfPatch[facei] = hBarPvol_.BOUNDARY_FIELD_REF[patchi][facei]
                      + ((hBarPgrad_.BOUNDARY_FIELD_REF[patchi][facei])
                       &(CfPatch[facei] - mesh_.C().boundaryField()[patchi][facei] - 0.5*xii*dt));
                    rSurfPatch[facei] = rBarPvol_.BOUNDARY_FIELD_REF[patchi][facei]
                      + ((rBarPgrad_.BOUNDARY_FIELD_REF[patchi][facei])
                       &(CfPatch[facei] - mesh_.C().boundaryField()[patchi][facei] - 0.5*xii*dt));   
                }
                else 
                {
                    ...
                    rSurfPatch[facei] = 0.5*(
                            iRbarPvol[faceCells[facei]] + 
                            (   (iRbarPgrad[faceCells[facei]]) & (CfPatch[facei] - C[faceCells[facei]] - 0.5*xii*dt) )
                            + rBarPvol_.BOUNDARY_FIELD_REF[patchi][facei] + 
                            (   (rBarPgrad_.BOUNDARY_FIELD_REF[patchi][facei]) & (CfPatch[facei] - mesh_.C().boundaryField()[patchi][facei] - 0.5*xii*dt) )
                    );                    
...
void Foam::discreteVelocity::updateGHbarSurfMaxwellWallIn()
{
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        if (gSurf_.BOUNDARY_FIELD_REF[patchi].type() == "maxwellWall")
        {
            ...
            fvsPatchScalarField& rSurfPatch = rSurf_.BOUNDARY_FIELD_REF[patchi];
            const fvPatchScalarField& rhoVolPatch = 
                dvm_.rhoVol().BOUNDARY_FIELD_REF[patchi];
            const fvPatchVectorField& UvolPatch = 
                dvm_.Uvol().BOUNDARY_FIELD_REF[patchi];
            const fvPatchScalarField& TvolPatch = 
                dvm_.Ttvol().BOUNDARY_FIELD_REF[patchi];
            ...
            forAll(gSurfPatch, facei)
            {
                ...
                if ((xii & faceSf) <= 0) // incomming
                {
                    ...
                    hSurfPatch[facei] = 
                        gSurfPatch[facei]*(dvm_.R().value()*TvolPatch[facei])*(3 - mesh_.nSolutionD());
                    rSurfPatch[facei] = 
                        gSurfPatch[facei]*(dvm_.R().value()*TvolPatch[facei]);
                }
            }
...
void Foam::discreteVelocity::updateGHsurf()
{
    ...
    (
        ...
        mesh_,
        dimensionedScalar("0", rTildeVol_.dimensions(), 0)
    );
    ...
    rSurf_ = (1.0 - relaxFactor)*rSurf_ + relaxFactor*rEq;
    // NOTE: here the boundary face value are not computed
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        fvsPatchScalarField& rEqPatch   =    rEq.BOUNDARY_FIELD_REF[patchi];

        const fvsPatchVectorField& SfPatch = mesh_.Sf().boundaryField()[patchi];

        fvsPatchScalarField& relaxFactorPatch = relaxFactor.BOUNDARY_FIELD_REF[patchi];
        ...
        forAll(gSurfPatch, facei)
        {
            ...
            //&& gSurf_.BOUNDARY_FIELD_REF[patchi].type() != "DVMsymmetry")
            {
                ...
                rSurfPatch[facei] = (1.0 - relaxFactorPatch[facei])
                    *rSurfPatch[facei]
                   + relaxFactorPatch[facei]*rEqPatch[facei];
            }
        }
        ...
        if(gSurfPatch.type() == "DVMsymmetry")
        {
            ...
            rSurfPatch = (1.0 - relaxFactorPatch)
                *rSurfPatch
               + relaxFactorPatch*rEqPatch;
        }
    }
}
void Foam::discreteVelocity::updateFlux()       /// update micro flux
{
    ...
    rFlux_ = dimensionedScalar("0", rFlux_.dimensions(), 0);
    ...
    forAll(owner, facei)
    {
        ...
        gFlux_[own] += ((xii&Sf[facei])*gSurf_[facei]*dt/V[own]);
        gFlux_[nei] -= ((xii&Sf[facei])*gSurf_[facei]*dt/V[nei]);
        hFlux_[own] += ((xii&Sf[facei])*hSurf_[facei]*dt/V[own]);
        hFlux_[nei] -= ((xii&Sf[facei])*hSurf_[facei]*dt/V[nei]);
        rFlux_[own] += ((xii&Sf[facei])*rSurf_[facei]*dt/V[own]);
        rFlux_[nei] -= ((xii&Sf[facei])*rSurf_[facei]*dt/V[nei]);
    }
    ...
    forAll(gSurf_.BOUNDARY_FIELD_REF, patchi)
    {
        ...
        const fvsPatchField<scalar>& rSurfPatch =
            rSurf_.BOUNDARY_FIELD_REF[patchi];    
        const fvsPatchField<vector>& SfPatch =
            mesh_.Sf().boundaryField()[patchi];
        const labelUList& pOwner = mesh_.boundary()[patchi].faceCells();
        forAll(pOwner, pFacei)
        {
            ...
            gFlux_[own] += (xii&SfPatch[pFacei]) 
               *gSurfPatch[pFacei]*dt/V[own];
            hFlux_[own] += (xii&SfPatch[pFacei]) 
               *hSurfPatch[pFacei]*dt/V[own];
            rFlux_[own] += (xii&SfPatch[pFacei]) 
               *rSurfPatch[pFacei]*dt/V[own];
        }
    }
}
...
void Foam::discreteVelocity::updateGHVol()
{
    ...
    (
        ...
        mesh_,
        dimensionedScalar("0", hVol_.dimensions(), 0)
    );
    ...
    (
        ...
        mesh_,
        dimensionedScalar("0", rVol_.dimensions(), 0)
    );
    ...
    hTildeVol_ = hEq;
    rTildeVol_ = rEq;
    gVol_ = relaxFactor*(gVol_ - gFlux_ + h*(gEq/dvm_.tauVol() + (geq - gVol_)/dvm_.tau()));
    hVol_ = relaxFactor*(hVol_ - hFlux_ + h*(hEq/dvm_.tauVol() + (heq - hVol_)/dvm_.tau()));
    rVol_ = relaxFactor*(rVol_ - rFlux_ + h*(rEq/dvm_.tauVol() + (req - rVol_)/dvm_.tau()));
}
...
template <template<class> class PatchType, class GeoMesh> 
void Foam::discreteVelocity::equilibriumRykov
(
    GeometricField<scalar, PatchType, GeoMesh>& gEq,
    GeometricField<scalar, PatchType, GeoMesh>& hEq,
    GeometricField<scalar, PatchType, GeoMesh>& rEq,
    const GeometricField<scalar, PatchType, GeoMesh>& rho,
    const GeometricField<vector, PatchType, GeoMesh>& U,
    const GeometricField<scalar, PatchType, GeoMesh>& T,
    const GeometricField<scalar, PatchType, GeoMesh>& Tt,
    const GeometricField<scalar, PatchType, GeoMesh>& Tr,
    const GeometricField<vector, PatchType, GeoMesh>& qt,
    const GeometricField<vector, PatchType, GeoMesh>& qr
)
{
    dimensionedScalar R = dvm_.R();
    //dimensionedScalar Pr = dvm_.Pr();
    dimensionedScalar Zr = 1.0/dvm_.Z();
    dimensionedScalar Zt = (1.0 - 1.0/dvm_.Z());
    dimensionedScalar omega0 = dvm_.omega0();
    dimensionedScalar omega1 = dvm_.omega1();
    dimensionedScalar delta = dvm_.delta();
    ...
    GeometricField<scalar, PatchType, GeoMesh> cSqrByRTt
        = magSqr(U - xi_)/(R*Tt);

    GeometricField<scalar, PatchType, GeoMesh> cqtBy15pRT
        = ((xi_ - U)&qt)/(15.0*rho*R*T*R*T);

    GeometricField<scalar, PatchType, GeoMesh> cqtBy15ptRTt
        = ((xi_ - U)&qt)/(15.0*rho*R*Tt*R*Tt);

    GeometricField<scalar, PatchType, GeoMesh> cqrBypRT
        = ((xi_ - U)&qr)/(rho*R*T*R*T);
    
    GeometricField<scalar, PatchType, GeoMesh> cqrByprRTt
        = ((xi_ - U)&qr)/(rho*R*Tr*R*Tt);

    GeometricField<scalar, PatchType, GeoMesh> gEqBGKT 
        = rho/pow(sqrt(2.0*pi*R*T),D)*exp(-cSqrByRT/2.0)/pow(vUnit, 3-D);

    GeometricField<scalar, PatchType, GeoMesh> gEqBGKTt 
        = rho/pow(sqrt(2.0*pi*R*Tt),D)*exp(-cSqrByRTt/2.0)/pow(vUnit, 3-D);

    GeometricField<scalar, PatchType, GeoMesh> gEqr
        =  gEqBGKT * (1.0 + omega0*cqtBy15pRT*(cSqrByRT - D - 2));

    GeometricField<scalar, PatchType, GeoMesh> gEqt
        =  gEqBGKTt * (1.0 + cqtBy15ptRTt*(cSqrByRTt - D - 2));

    gEq = Zr * gEqr + Zt * gEqt;

    hEq = Zr *R*T*gEqBGKT * ((3-D)+omega0*cqtBy15pRT*(cSqrByRT - D)*(3-D)) +
          Zt *R*Tt*gEqBGKTt * ((3-D)+cqtBy15ptRTt*(cSqrByRTt - D)*(3-D));

    rEq = Zr *R*T*(gEqr + omega1*(1-delta)*cqrBypRT*gEqBGKT) + 
          Zt *R*Tr*(gEqt + (1-delta)*cqrByprRTt*gEqBGKTt);
}
