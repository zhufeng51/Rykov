...
int main(int argc, char *argv[])
{
    ...
    scalar TemperatureChange1 = 1.0;
    scalar TemperatureChange2 = 1.0;
    ...
    Info << "The mesh dimensionality is: " << mesh.nSolutionD() << endl;

    fvDVM dvm(rho, U, T, Tt, Tr, &argc, &argv, args);
    ...
    while (runTime.run() && (Uchange > convergeTol))
    {
        ...
        if(It%convergeCheckSteps == 0 && It >= convergeCheckSteps)
        {
                tmp<Foam::GeometricField<scalar, Foam::fvPatchField, Foam::volMesh> > 
                    deltaTem = mag(T-Told);
                tmp<Foam::GeometricField<scalar, Foam::fvPatchField, Foam::volMesh> > 
                    deltaTem1 = mag(Tt-Ttold);
                tmp<Foam::GeometricField<scalar, Foam::fvPatchField, Foam::volMesh> > 
                    deltaTem2 = mag(Tr-Trold);
                tmp<Foam::GeometricField<scalar, Foam::fvPatchField, Foam::volMesh> > 
                ...
                TemperatureChange1 = gSum(deltaTem1())/gSum(Tt);
                TemperatureChange2 = gSum(deltaTem2())/gSum(Tr);
                rhoChange         = gSum(deltaRho())/gSum(rho);
                Uchange           = gSum(deltaU())/gSum(mag(U)());

                Info << "Temperature       changes = " << TemperatureChange  << endl;
                Info << "Trans Temperature changes = " << TemperatureChange1 << endl;
                Info << "Rot  Temperature  changes = " << TemperatureChange2 << endl;
